...
### IOC, DI ?
- IoC : 제어의 역전
  - 기존에는 객체가 직접 다른 객체를 생성하고 관리했다.(제어의 순방향) 
  - IOC를 사용하면 객체 생성과 관리를 개발자가 아닌 프레임워크가 맡게 된다.(제어의 역전)
    - 내가 만든 프로그램이 이미 만들어져 있는 공장(프레임워크) 안의 요소로 들어가 하나의 톱니바퀴처럼 동작
  - 클래스 간 결합을 느슨히 하기 위함
    - 한 클래스의 변경에 따른 다른 클래스들의 영향을 최소화
  - Ioc는 제어권이 내부에서 외부로 역전된 것 -> 제어의 흐름이 역전
  - 원칙
- DI : 의존성 주입
  - IoC를 구현하기 위한 하나의 방법 -> 디자인 패턴
  - 클래스 간에 의존 관계가 있다는 것은?
    - 한 클래스가 바뀔 때 다른 클래스가 영향을 받는 것
  - 외부(제3 자)에서 객체를 주입해준다는 구체적인 행위.
...
- 객체(빈)를 직접적으로 생성, 연관된 의존성을 주입, 생명주기를 관리한다.
- 애플리케이션 컴포넌트의 중앙 저장소
